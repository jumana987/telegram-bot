...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "⚡ Fast • Simple • Premium\n\n"
        "👤 Owner: @Momy_A1\n\n"
        "Tap <b>Get Started</b> to continue.",
        parse_mode="HTML",
...
def main():
    ...
    app.add_handler(
        CallbackQueryHandler(button_handler, pattern=r"^(start_number|help)$")
    )
    # Railway-friendly webhook mode: avoids Telegram getUpdates polling conflicts.
    ...
    if not public_url:
        raise RuntimeError(
            "RAILWAY_PUBLIC_DOMAIN or PUBLIC_URL environment variable is missing."
        )
